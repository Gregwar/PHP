...
.. step::
    ...
    Commencez par installer le module de sécurité::

        composer req security

    Vous pouvez également installer le *profiler*, qui est un outil de débogage pratique
    dans Symfony::

        composer req profiler

    Vous pouvez constater que la barre qui apparaîtra alors en bas de vos pages contient
    l'utilisateur identifié (ou anon. pour "anonyme").
...
.. step::
    Premièrement, nous allons activer le sytème de session. Pour ce faire, décommentez
    la partie concernant les sessions dans ``app/config/packages/framework.yml``.

    Ensuite, il vous faudra configurer le module de sécurité de Symfony. Pour cela,
    éditez le fichier ``app/config/packages/security.yml``.
    ...
    * Définit un filtrage d'accès sur les URLs commençant par ``/admin/``, il faudra le role
    ...
    Créez ensuite des routes dans ``app/config/routes.yaml`` pour les actions ``login_check`` et
    ``logout``::
